...
export default function ControlsBar({
  params,                 // { VDD, CLOAD, TSTEP, TSTOP, TEMP }
  onChange,               // (key, value) => void
  onRun,                  // () => void
  auto, setAuto,          // boolean, setter
  onPresetChange,         // (presetObj) => void
  onUploadFileText,       // (fileText) => void
  fileName,               // NEW: uploaded .cir filename to show next to upload icon
}) {
  return (
    <header className="sticky top-0 z-40 bg-slate-950/75 backdrop-blur supports-[backdrop-filter]:bg-slate-950/60 border-b-[1.5px] border-emerald-500/50">
      <div className="w-full py-3 flex flex-wrap items-center gap-3 justify-start">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* filename badge (optional) */}
          {fileName ? (
            <span
              title={fileName}
              className="max-w-[220px] truncate px-2 py-1 rounded-md border border-slate-700/70 bg-slate-900/70 text-[12px] text-slate-300"
            >
              {fileName}
            </span>
          ) : null}
